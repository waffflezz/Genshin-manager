...
def grab_notes(uid, lang):
    ...
    info = res_text['info']
    info['dailik'] = f"{response['completed_commissions']}\\" \
    ...
    info['reward'] = f"{is_claimed}"
    info['bosses'] = f"{response['remaining_boss_discounts']}\\{response['max_boss_discounts']}"

    info['resin'] = {'amount': f"{response['resin']}\\{response['max_resin']}\n",
                     'time': f"{get_time_from_sec(response['until_resin_limit'], lang)}"}

    info['expedition'] = f"{len(list(filter(lambda expa: expa['remaining_time'] != 0, response['expeditions'])))}" \
                         f"\\{response['max_expeditions']}"

    for exp in response['expeditions']:
        res_text['characters'].append({'img': get_img_from_web(exp['icon']),
        # res_text['characters'].append({'img': exp['icon'],
